...
import { Search, SlidersHorizontal, Star, Home, Compass, ClipboardList, User, ShoppingBag, LogOut, Settings, Bell } from 'lucide-react';
...
const UserHome = () => {
  ...
  const handleSearch = async (e) => {
    ...
    try {
      ...
      const response = await axios.get(`http://localhost:5000/api/user/search?query=${value}`);
      setServices(response.data.services);
    } catch (err) {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <header className="p-4 md:p-8 flex justify-between items-center sticky top-0 bg-[#FDF8F1]/80 backdrop-blur-md z-40">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
            {/* <div className="w-10 h-10 bg-orange-700 rounded-xl flex items-center justify-center shadow-lg shadow-orange-200">
               <span className="text-white font-black text-xl">T</span>
            </div> */}
            <h1 className="text-xl md:text-2xl font-heading font-bold text-orange-900 hidden sm:block">Tiffin Finder</h1>
          </div>
            ...
            <div className="relative">
              ...
              {isProfileOpen && (
                <>
                  ...
                  <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl shadow-orange-200/50 border border-orange-50 z-20 overflow-hidden animate-in fade-in zoom-in duration-200 font-modern">
                    <div className="px-4 py-4 bg-orange-50/50">
                      <p className="text-sm font-bold text-gray-800">Foodie Account</p>
                      {/* <p className="text-[10px] text-orange-600 font-black uppercase tracking-widest">Premium User</p> */}
                    </div>
                    <div className="p-1">
        ...
        {/* --- SEARCH & FILTERS --- */}
        <div className="md:flex md:items-center md:gap-4 px-4 md:px-8 py-4 sticky top-16 md:top-24 z-30">
        ...
        <main className="px-4 md:px-8 py-10">
          <div className="flex justify-between items-center mb-10">
            <h3 className="text-2xl font-heading font-bold text-gray-800">
              {searchQuery ? `Results for "${searchQuery}"` : "Nearby Home Kitchens"}
            </h3>
          ...
          {!loading && services.length === 0 && (
            <div className="text-center py-20 bg-white rounded-[40px] border border-dashed border-orange-200">
                <p className="text-gray-400 font-medium text-lg">No providers found in this area.</p>
                <button onClick={fetchServices} className="mt-4 text-orange-600 font-bold underline">Clear filters</button>
            </div>
          )}
        </main>
      </div>
