...
import { Search, Bell, Plus, Star, Edit2, Menu, X, Trash2, LogOut, User, Settings } from 'lucide-react';
      ...
      <nav className="sticky top-0 z-50 bg-[#FDF8F1]/80 backdrop-blur-md border-b border-orange-100 px-4 md:px-8 py-3">
        <div className="max-w-7xl mx-auto flex justify-end items-center">
          ...
          {/* <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate('/')}>
            ...
            <span className="font-heading text-xl font-bold text-gray-800 hidden sm:block tracking-tight">ChefStream</span>
          </div> */}

          {/* Right Side Actions */}
          <div className="flex  items-center gap-3 md:gap-5">
            <button className="p-2 text-gray-500 hover:bg-orange-100 rounded-full transition-colors relative">
            ...
            <div className="relative">
              ...
              >
                ...
                <div className="hidden md:block text-left mr-2">
                  <p className="text-xs font-bold text-gray-800 leading-none">Chef Admin</p>
                  <p className="text-[10px] text-orange-600 font-medium">Provider</p>
                </div>
              </button>
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 mt-10">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
          ...
          <div className="flex items-center gap-3">
            <div className="bg-white px-5 py-2.5 rounded-2xl border border-orange-100 shadow-sm">
               <span className="text-[10px] uppercase tracking-widest font-black text-gray-400 block mb-0.5">Total Services</span>
               <span className="text-xl font-heading font-bold text-[#A63C13]">{services.length}</span>
            </div>
          </div>
        </header>
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32">
            ...
            <p className="mt-4 text-gray-400 font-medium animate-pulse">Entering the kitchen...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            >
              ...
              <h4 className="font-heading font-bold text-xl mt-6 mb-2">New Listing</h4>
              <p className="text-sm text-gray-400 max-w-[180px]">Add a new meal plan or catering service</p>
            </div>
          </div>
        )}
      </main>
...
const ServiceCard = ({ _id, title, pricePerMonth, rating, image, mealType, onDelete, onEdit }) => {
  const navigate = useNavigate();
    ...
    >
      <div className="relative aspect-[16/11] overflow-hidden">
        ...
        <div className="absolute bottom-4 left-4 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full flex items-center gap-1 shadow-sm">
          ...
          <span className="text-xs font-bold text-gray-800">{rating || "New"}</span>
        </div>
      </div>
      
      <div className="p-6 flex flex-col flex-grow">
        <div className="flex justify-between items-start gap-2 mb-2">
          <h3 className="font-heading font-bold text-xl text-gray-900 line-clamp-1 leading-tight">{title}</h3>
        </div>
        
        <div className="mt-auto pt-6 flex justify-between items-end">
          <div className="flex flex-col">
            <span className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-1">Monthly</span>
            <span className="text-2xl font-heading font-black text-[#A63C13]">₹{pricePerMonth}</span>
          </div>
